...
###### Default parameters
**JavaScript** очень свободно относится к количеству переданных в функцию аргументов, что с одной стороны хорошо, однако, в отличии от других языков **ES 5** не предоставляет возможности определить значения по умолчанию в сигнатуре функции. 
...
В **ES 5** для этого можно использовать метод ``apply``, что не очень оптимально и не удобно, например:
...
В **ES2015** выполнение такой задачи немного проще, так как для массивов определен служебный метод ``Array.prototype[Symbol.iterator]``, для объектов такой метод не определен, потому использование ``for..of`` для объектов возможно только, если определить итератор самостоятельно, мы посмотрим пример итератора позже.
    ...
    <div class="medium-12 columns">
        ...
        <ul class="list-nostyle post-list">
            <li class="post-list__item border-bottom">
                <a href="http://www.webcamp.com.ua/courses/js_advanced">
                    <p class="p-5">JavaScript Advanced</p>
                </a>
            </li>
            <li class="post-list__item border-bottom">
                <a href="http://www.webcamp.com.ua/courses/js_angular">
                    <p class="p-5">Angular JS</p>
                </a>
            </li>
        </ul>
    </div>
