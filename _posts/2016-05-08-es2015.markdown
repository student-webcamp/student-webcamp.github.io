...
### <span class="icon-homecode" id="for_of"></span> Цикл for..of
...
В **ES2015** выполнение такой задачи немного проще, так как для массивов определен служебный метод ``Array.prototype[Symbol.iterator]``, для объектов такой метод не определен, потому использование ``for..of`` для объктов возможно только, если опрелить итератор самостоятельно, мы посмотрим пример итератора позже.
